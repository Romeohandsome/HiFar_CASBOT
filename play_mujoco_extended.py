...
class MujocoRunner:
    ...
    def play(self):
        ...
        effective_ids = [0, 3, 4, 7, 9, 10, 12, 13, 15, 16, 18, 19]
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            ...
            while viewer.is_running():
                ...
                if it % (self.cfg["control"]["decimation"] * self.cfg["sim"]["substeps"]) == 0:
                    ...
                    obs[0, 6:18] = (dof_pos - default_dof_pos)[effective_ids] * self.cfg["normalization"]["dof_pos"]
                    obs[0, 18:30] = dof_vel[effective_ids] * self.cfg["normalization"]["dof_vel"]
                    obs[0, 30:42] = actions
                    if it == 0:
    ...
    @staticmethod
    def mirror_obs(obs):
        mat = np.zeros((42, 42), dtype=np.float32)
        mat[ 0: 6,  0: 6] = np.eye(6)
        mat[ 6: 8,  8:10] = np.eye(2)
        mat[ 8:10,  6: 8] = np.eye(2)
        mat[10:14, 14:18] = np.eye(4)
        mat[14:18, 10:14] = np.eye(4)
        mat[18:20, 20:22] = np.eye(2)
        mat[20:22, 18:20] = np.eye(2)
        mat[22:26, 26:30] = np.eye(4)
        mat[26:30, 22:26] = np.eye(4)
        mat[30:32, 32:34] = np.eye(2)
        mat[32:34, 30:32] = np.eye(2)
        mat[34:38, 38:42] = np.eye(4)
        mat[38:42, 34:38] = np.eye(4)
        flip_val = np.ones(42, dtype=np.float32)
        inverse_ids = [  1,  3,  5,
                         7,  9, 11, 15,
                        19, 21, 23, 27,
                        31, 33, 35, 39]
    ...
    @staticmethod
    def mirror_act(act):
        mat = np.zeros((12, 12), dtype=np.float32)
        mat[0:2, 2:4] = np.eye(2)
        mat[2:4, 0:2] = np.eye(2)
        mat[4:8, 8:12] = np.eye(4)
        mat[8:12, 4:8] = np.eye(4)
        flip_val = np.ones(12, dtype=np.float32)
        inverse_ids = [1, 3]
        flip_val[inverse_ids] = -1
        flip_mat = np.diag(flip_val)
        mirror_transform_mat = np.dot(mat, flip_mat)
