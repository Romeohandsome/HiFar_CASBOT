...
class T1FallRecoveryRandom(BaseTask):
    ...
    @staticmethod
    def mirror_obs(obs):
        mat = torch.zeros(42, 42, dtype=torch.float, device=obs.device)
        mat[ 0: 6,  0: 6] = torch.eye(6)
        mat[ 6: 8,  8:10] = torch.eye(2)
        mat[ 8:10,  6: 8] = torch.eye(2)
        mat[10:14, 14:18] = torch.eye(4)
        mat[14:18, 10:14] = torch.eye(4)
        mat[18:20, 20:22] = torch.eye(2)
        mat[20:22, 18:20] = torch.eye(2)
        mat[22:26, 26:30] = torch.eye(4)
        mat[26:30, 22:26] = torch.eye(4)
        mat[30:32, 32:34] = torch.eye(2)
        mat[32:34, 30:32] = torch.eye(2)
        mat[34:38, 38:42] = torch.eye(4)
        mat[38:42, 34:38] = torch.eye(4)
        flip_val = torch.ones(42, dtype=torch.float, device=obs.device)
        inverse_ids = [  1,  3,  5,
                         7,  9, 11, 15,
                        19, 21, 23, 27,
                        31, 33, 35, 39]
    ...
    @staticmethod
    def mirror_act(act):
        mat = torch.zeros(12, 12, dtype=torch.float, device=act.device)
        mat[0:2, 2:4] = torch.eye(2)
        mat[2:4, 0:2] = torch.eye(2)
        mat[4:8, 8:12] = torch.eye(4)
        mat[8:12, 4:8] = torch.eye(4)
        flip_val = torch.ones(12, dtype=torch.float, device=act.device)
        inverse_ids = [1, 3, 5, 9]
        flip_val[inverse_ids] = -1
        flip_mat = torch.diag(flip_val)
        mirror_transform_mat = torch.matmul(mat, flip_mat)
    ...
    def _reward_hip_roll(self):
        # Penalize hip roll positions away from reference
        ref_loss = torch.square(self.dof_pos_ref - self.dof_pos)[:, 12] + torch.square(self.dof_pos_ref - self.dof_pos)[:, 18]
        return ref_loss
