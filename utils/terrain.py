...
class Terrain:
    ...
    def create_trimesh(self):
        ...
        self.height_field_raw = np.zeros((self.terrain_cfg["num_terrains"] * terrain_width_pixels + 2 * self.border_pixels, terrain_length_pixels + 2 * self.border_pixels), dtype=np.int16)
        proportions = [
            self.terrain_cfg["num_terrains"] * np.sum(self.terrain_cfg["terrain_proportions"][: i + 1]) / np.sum(self.terrain_cfg["terrain_proportions"])
            for i in range(len(self.terrain_cfg["terrain_proportions"]))
        ]
        for i in range(self.terrain_cfg["num_terrains"]):
            terrain = terrain_utils.SubTerrain("terrain", width=terrain_width_pixels, length=terrain_length_pixels, vertical_scale=self.vertical_scale, horizontal_scale=self.horizontal_scale)
            if i < proportions[0]:
                pass
                ...
                terrain_utils.random_uniform_terrain(terrain, min_height=-0.5 * self.terrain_cfg["random_height"], max_height=0.5 * self.terrain_cfg["random_height"], step=0.005, downsampled_scale=0.2)
            else:
                terrain_utils.discrete_obstacles_terrain(terrain, max_height=self.terrain_cfg["discrete_height"], min_size=1.0, max_size=2.0, num_rects=20, platform_size=3.0)
        ...
        vertices, triangles = terrain_utils.convert_heightfield_to_trimesh(self.height_field_raw, self.horizontal_scale, self.vertical_scale, self.terrain_cfg["slope_threshold"])
    ...
    def terrain_heights(self, base_pos):
        if self.type == "plane":
            return torch.zeros(len(base_pos), dtype=torch.float, device=self.device, requires_grad=False)
        else:
